...
@Component({
  selector: 'jhi-customer-transaction',
...
export class CustomerTransactionComponent implements OnInit, OnDestroy {
  ...
  loadAll() {
    this.customerTransactionService
      ...
      .subscribe((res: HttpResponse<ICustomerTransaction[]>) => this.paginateCustomerTransactions(res.body, res.headers));
  }
  ...
  trackId(index: number, item: ICustomerTransaction) {
    return item.id;
  }

  registerChangeInCustomerTransactions() {
    this.eventSubscriber = this.eventManager.subscribe('customerTransactionListModification', response => this.loadAll());
  }
  ...
  protected paginateCustomerTransactions(data: ICustomerTransaction[], headers: HttpHeaders) {
