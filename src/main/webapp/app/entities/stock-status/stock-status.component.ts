...
export class StockStatusComponent implements OnInit, OnDestroy {
  ...
  loadAll() {
    this.stockStatusService
      ...
      .subscribe((res: HttpResponse<IStockStatus[]>) => this.paginateStockStatuses(res.body, res.headers));
  }
  ...
  registerChangeInStockStatuses() {
    this.eventSubscriber = this.eventManager.subscribe('stockStatusListModification', response => this.loadAll());
  }
