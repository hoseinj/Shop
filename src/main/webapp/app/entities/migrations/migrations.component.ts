...
export class MigrationsComponent implements OnInit, OnDestroy {
  ...
  loadAll() {
    this.migrationsService
      ...
      .subscribe((res: HttpResponse<IMigrations[]>) => this.paginateMigrations(res.body, res.headers));
  }
  ...
  trackId(index: number, item: IMigrations) {
    return item.id;
  }

  registerChangeInMigrations() {
    this.eventSubscriber = this.eventManager.subscribe('migrationsListModification', response => this.loadAll());
  }
  ...
  protected paginateMigrations(data: IMigrations[], headers: HttpHeaders) {
